...
function ensure_guest_record($con, $ip, $guestId) {
    ...
    $ipSafe = mysqli_real_escape_string($con, $ip);
    $idSafe = mysqli_real_escape_string($con, $guestId);
    $res = mysqli_query($con, "SELECT * FROM uso_convidado WHERE endereco_ip = '$ipSafe' AND id_anonimo = '$idSafe' LIMIT 1");
    ...
    $stmt = mysqli_prepare($con, "INSERT INTO uso_convidado (endereco_ip, id_anonimo, total_pedidos, data_primeiro_pedido, data_ultimo_pedido, data_expiracao, bloqueado, data_criacao, data_atualizacao) VALUES (?, ?, 0, ?, ?, ?, 0, ?, ?)");
    if ($stmt) {
        mysqli_stmt_bind_param($stmt, 'sssssss', $ip, $guestId, $now, $now, $exp, $now, $now);
        mysqli_stmt_execute($stmt);
    } else {
        // Try fallback insert
        mysqli_query($con, "INSERT INTO uso_convidado (endereco_ip, id_anonimo, total_pedidos, data_primeiro_pedido, data_ultimo_pedido, data_expiracao, bloqueado, data_criacao, data_atualizacao) VALUES ('$ipSafe','$idSafe',0,'$now','$now','$exp',0,'$now','$now')");
    }
    $res2 = mysqli_query($con, "SELECT * FROM uso_convidado WHERE endereco_ip = '$ipSafe' AND id_anonimo = '$idSafe' LIMIT 1");
...
function guest_check_and_increment($con) {
    ...
    if (!empty($rec['data_expiracao']) && strtotime($rec['data_expiracao']) < time()) {
        mysqli_query($con, "UPDATE uso_convidado SET total_pedidos = 0, data_primeiro_pedido = '$now', data_ultimo_pedido = '$now', data_expiracao = '" . date('Y-m-d H:i:s', time() + 7*24*3600) . "', bloqueado = 0, data_atualizacao = '$now' WHERE id = " . ((int)$rec['id'])) ;
    ...
    if (!empty($rec['bloqueado']) || (int)$rec['total_pedidos'] >= GUEST_MAX_FREE) {
        ...
        mysqli_query($con, "UPDATE uso_convidado SET bloqueado = 1 WHERE id = " . ((int)$rec['id']));
    ...
    $stmt = mysqli_prepare($con, "UPDATE uso_convidado SET total_pedidos = ?, data_ultimo_pedido = ?, data_atualizacao = ? WHERE id = ?");
    if ($stmt) {
        $idInt = (int)$rec['id'];
        $nowSql = date('Y-m-d H:i:s');
        mysqli_stmt_bind_param($stmt, 'issi', $newTotal, $nowSql, $nowSql, $idInt);
        mysqli_stmt_execute($stmt);
    } else {
        mysqli_query($con, "UPDATE uso_convidado SET total_pedidos = $newTotal, data_ultimo_pedido = '$now', data_atualizacao = '$now' WHERE id = " . ((int)$rec['id']));
    }
