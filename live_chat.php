...
function verificarAutenticacao()
{
    ...
    $email = trim($email);
    $stmt = mysqli_prepare($con, "SELECT id_utilizador, nome FROM utilizador WHERE email = ? LIMIT 1");
...
function enviarMensagem($dados, $utilizadorId)
{
    ...
    if (!$conversationId || !is_numeric($conversationId)) {
        $stmtChat = mysqli_prepare($con, "INSERT INTO chat_ajuda (id_utilizador, criado_em) VALUES (?, NOW())");
        if (!$stmtChat) {
            responderErro('Erro ao criar conversa');
        }
        mysqli_stmt_bind_param($stmtChat, "i", $utilizadorId);
        mysqli_stmt_execute($stmtChat);
        $conversationId = mysqli_insert_id($con);
        mysqli_stmt_close($stmtChat);
    }

    $stmtMsg = mysqli_prepare($con, "INSERT INTO mensagens_chat_ajuda (conversation_id, sender, conteudo, enviado_em) VALUES (?, 'utilizador', ?, NOW())");
    if (!$stmtMsg) {
        responderErro('Erro ao enviar mensagem');
    }

    mysqli_stmt_bind_param($stmtMsg, "is", $conversationId, $conteudo);
...
function obterConversa($conversationId)
{
    ...
    $stmtConv = mysqli_prepare($con, "SELECT id_utilizador, estado FROM chat_ajuda WHERE id = ? LIMIT 1");
    if (!$stmtConv) {
        responderErro('Erro ao verificar conversa');
    }

    mysqli_stmt_bind_param($stmtConv, "i", $conversationId);
    mysqli_stmt_execute($stmtConv);
    $resultConv = mysqli_stmt_get_result($stmtConv);

    if (mysqli_num_rows($resultConv) === 0) {
        responderErro('Conversa não encontrada');
    }

    $rowConv = mysqli_fetch_assoc($resultConv);
    mysqli_stmt_close($stmtConv);
...
function fecharConversaUtilizador($dados, $utilizadorId)
{
    ...
    $stmt = mysqli_prepare($con, "SELECT id_utilizador FROM chat_ajuda WHERE id = ? LIMIT 1");
    if (!$stmt) {
        responderErro('Erro ao verificar conversa');
    }
    ...
    $updateStmt = mysqli_prepare($con, "UPDATE chat_ajuda SET estado = 'fechado' WHERE id = ?");
    if (!$updateStmt) {
    ...
    if (!mysqli_stmt_execute($updateStmt)) {
        responderErro('Erro ao fechar conversa: ' . mysqli_stmt_error($updateStmt));
    }
