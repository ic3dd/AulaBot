...
$resultadoUpdate = mysqli_query($con, "SELECT id_update, nome, versao, descricao, data_update FROM updates ORDER BY data_update DESC LIMIT 1");
if ($resultadoUpdate && mysqli_num_rows($resultadoUpdate) > 0) {
    $update = mysqli_fetch_assoc($resultadoUpdate);
    echo "<pre>";
    print_r($update);
    echo "</pre>";
    ...
    echo "Resultados: " . mysqli_num_rows($resultadoVerificar) . "<br>";
    ...
    $resultadoVerificar24h = mysqli_query($con, $queryVerificar24h);
    echo "Query: $queryVerificar24h<br>";
    echo "Resultados nas últimas 24h: " . mysqli_num_rows($resultadoVerificar24h) . "<br>";
    
    if (mysqli_num_rows($resultadoVerificar24h) === 0) {
    ...
    echo "Erro MySQL: " . mysqli_error($con) . "<br>";
}
