...
{
    db_query($con, "CREATE TABLE IF NOT EXISTS `chats` (`id_chat` int AUTO_INCREMENT PRIMARY KEY, `id_utilizador` int NOT NULL, `titulo` varchar(255), `data_criacao_chat` datetime, `data_atualizacao` datetime) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;");
    db_query($con, "CREATE TABLE IF NOT EXISTS `mensagens` (`id_mensagem` int AUTO_INCREMENT PRIMARY KEY, `id_chat` int NOT NULL, `pergunta` text, `resposta` text, `data_conversa` datetime, `id_imagem` int DEFAULT NULL) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;");
    db_query($con, "CREATE TABLE IF NOT EXISTS `mensagens_imagem` (`id_imagem` int AUTO_INCREMENT PRIMARY KEY, `id_mensagem` int NOT NULL, `filename` varchar(255), `mime` varchar(100), `content` longblob, `data_insercao` datetime DEFAULT CURRENT_TIMESTAMP) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;");
    db_query($con, "CREATE TABLE IF NOT EXISTS `uso_convidado` (`id` int AUTO_INCREMENT PRIMARY KEY, `ip_hash` varchar(64), `id_anonimo` varchar(100), `total_pedidos` int DEFAULT 0, `data_primeiro_pedido` datetime, `data_ultimo_pedido` datetime, `data_expiracao` datetime, `bloqueado` tinyint DEFAULT 0, KEY `id_anonimo` (`id_anonimo`)) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;");
}
...
try {
    ...
    if ($idUtilizador === 0 && isset($_SESSION['email'])) {
        ...
        $stmt = db_prepare($con, "SELECT id_utilizador FROM utilizador WHERE email = ? LIMIT 1");
        db_stmt_bind_param($stmt, 's', $_SESSION['email']);
    ...
    if ($idUtilizador === 0) {
        ...
        $stmtIp = db_prepare($con, "SELECT * FROM uso_convidado WHERE ip_hash = ? ORDER BY id DESC LIMIT 1");
        db_stmt_bind_param($stmtIp, 's', $userIpHash);
        db_stmt_execute($stmtIp);
        $resIp = db_stmt_get_result($stmtIp);
        $guestRecord = db_fetch_assoc($resIp);
        ...
        if ($guestRecord) {
            ...
            if (!empty($anonymousId)) {
                $stmt = db_prepare($con, "SELECT * FROM uso_convidado WHERE id_anonimo = ? LIMIT 1");
                db_stmt_bind_param($stmt, 's', $anonymousId);
                db_stmt_execute($stmt);
                $res = db_stmt_get_result($stmt);
                $guestRecord = db_fetch_assoc($res);
            }
        }
        ...
        if ($guestRecord) {
            ...
            if (time() > strtotime($guestRecord['data_expiracao'])) {
                // UPDATE seguro
                $stmtReset = db_prepare($con, "UPDATE uso_convidado SET total_pedidos = 0, data_expiracao = " . db_sql_date_add('7 DAY') . ", bloqueado = 0 WHERE id = ?");
                db_stmt_bind_param($stmtReset, 'i', $guestRecord['id']);
                db_stmt_execute($stmtReset);
                db_stmt_close($stmtReset);
            ...
            $stmtInc = db_prepare($con, "UPDATE uso_convidado SET total_pedidos = total_pedidos + 1, data_ultimo_pedido = NOW() WHERE id = ?");
            db_stmt_bind_param($stmtInc, 'i', $guestRecord['id']);
            db_stmt_execute($stmtInc);
            db_stmt_close($stmtInc);

        } else {
            // Novo Visitante: Cria registo
            $newAnonymousId = gen_uuid();
            $stmt = db_prepare($con, "INSERT INTO uso_convidado (ip_hash, id_anonimo, total_pedidos, data_primeiro_pedido, data_ultimo_pedido, data_expiracao) VALUES (?, ?, 1, NOW(), NOW(), " . db_sql_date_add('7 DAY') . ")");
            db_stmt_bind_param($stmt, 'ss', $userIpHash, $newAnonymousId);
            db_stmt_execute($stmt);
        }
    }
...
3. ATENÇÃO: O OCR falha frequentemente em símbolos matemáticos. Verifica a coerência lógica.";
        ...
        if (!empty($chatId) && $idUtilizador > 0) {
            ...
            $stmtH = db_prepare($con, "SELECT pergunta, resposta FROM mensagens WHERE id_chat = ? ORDER BY data_conversa DESC LIMIT 20");
            db_stmt_bind_param($stmtH, 'i', $chatId);
            db_stmt_execute($stmtH);
            $resH = db_stmt_get_result($stmtH);
            $h = [];
            while ($r = db_fetch_assoc($resH)) {
    ...
    if ($idUtilizador > 0) {
        ...
        if (empty($chatId)) {
            ...
            $stmt = db_prepare($con, "INSERT INTO chats (id_utilizador, titulo, data_criacao_chat, data_atualizacao) VALUES (?, ?, NOW(), NOW())");

            if ($stmt) {
                db_stmt_bind_param($stmt, 'is', $idUtilizador, $titulo);
                if (db_stmt_execute($stmt)) {
                    $chatId = db_insert_id($con);
                }
                db_stmt_close($stmt);
            }
        } else {
            // UPDATA seguro
            $stmtUpdate = db_prepare($con, "UPDATE chats SET data_atualizacao = NOW() WHERE id_chat = ?");
            db_stmt_bind_param($stmtUpdate, 'i', $chatId);
        ...
        // B. Tabela MENSAGENS
        if ($chatId) {
            $stmt = db_prepare($con, "INSERT INTO mensagens (id_chat, pergunta, resposta, data_conversa) VALUES (?, ?, ?, NOW())");

            if ($stmt) {
                db_stmt_bind_param($stmt, 'iss', $chatId, $textoPrincipal, $fullReply);
                if (db_stmt_execute($stmt)) {
                    $idMsg = db_insert_id($con);

                    // C. IMAGENS (Vincula a imagem à mensagem se existir URL)
                    if (!empty($imageUrl) && $idMsg) {
                        try {
                            $stmtImg = db_prepare($con, "INSERT INTO mensagens_imagem (id_mensagem, filename, mime) VALUES (?, ?, ?)");
                            if ($stmtImg) {
                                $m = 'image/jpeg';
                                db_stmt_bind_param($stmtImg, 'iss', $idMsg, $imageUrl, $m);

                                if (db_stmt_execute($stmtImg)) {
                                    $idImagem = db_insert_id($con);
                                    // Atualiza a tabela mensagens com o ID da imagem inserida (Prepared Statement)
                                    $stmtUpdateMsg = db_prepare($con, "UPDATE mensagens SET id_imagem = ? WHERE id_mensagem = ?");
                                    db_stmt_bind_param($stmtUpdateMsg, 'ii', $idImagem, $idMsg);
                                    db_stmt_execute($stmtUpdateMsg);
                                    db_stmt_close($stmtUpdateMsg);
                                }
                                db_stmt_close($stmtImg);
                            }
                        } catch (Throwable $e) {
                    ...
                    $sqlError = "Erro Gravar Msg: " . db_stmt_error($stmt);
                }
                db_stmt_close($stmt);
            }
        }
