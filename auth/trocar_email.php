...
try {
    ...
    db_stmt_bind_param($stmt, "s", $email_atual);
    ...
    $stmtCheck = db_prepare($con, $checkEmail);
    
    if (!$stmtCheck) {
        throw new Exception('Erro ao preparar query de verificação: ' . db_error($con));
    }

    db_stmt_bind_param($stmtCheck, "ss", $novo_email, $email_atual);
    ...
    db_stmt_bind_param($stmt_update, "ss", $novo_email, $email_atual);
