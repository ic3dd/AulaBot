...
try {
    ...
    } elseif (!defined('DB_IS_POSTGRES') || !DB_IS_POSTGRES) {
        $checkColumn = db_query($con, "SELECT DATA_TYPE FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'utilizador' AND COLUMN_NAME = 'tema_escola'");
        if ($checkColumn && db_num_rows($checkColumn) > 0) {
            $row = db_fetch_assoc($checkColumn);
            if (strtolower($row['DATA_TYPE'] ?? '') !== 'longtext') {
                db_query($con, "ALTER TABLE utilizador MODIFY COLUMN tema_escola LONGTEXT DEFAULT NULL");
            }
        }
    }
    ...
    db_stmt_bind_param($stmt_update, "ss", $disciplina_json, $email);
