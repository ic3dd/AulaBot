...
$resultadoId = db_query($con, $queryId);

if (!$resultadoId || db_num_rows($resultadoId) === 0) {
    respondJson(false, 'Utilizador não encontrado no sistema.', 404);
}

$row = db_fetch_assoc($resultadoId);
...
if (defined('DB_IS_POSTGRES') && DB_IS_POSTGRES) {
    $query = "INSERT INTO anuncios_vistos (id_utilizador, id_update, data_visualizacao) 
              ...
              ON CONFLICT (id_utilizador, id_update) DO UPDATE SET data_visualizacao = NOW()";
} else {
    $query = "INSERT INTO anuncios_vistos (id_utilizador, id_update, data_visualizacao) 
              VALUES ($idUtilizador, $idUpdate, NOW())
              ON DUPLICATE KEY UPDATE data_visualizacao = NOW()";
}
$resultado = db_query($con, $query);
